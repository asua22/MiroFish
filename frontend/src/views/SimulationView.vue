...
const handleNextStep = (params = {}) => {
  ...
  // Pass custom rounds via query parameters if any
  if (params.maxRounds) {
 ...
 * When user returns to Step 2 from Step 3, default to exit simulation
 */
const checkAndStopRunningSimulation = async () => {
  if (!currentSimulationId.value) return
  
  try {
    // Check if simulation environment is alive first
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        // If graceful close errors, try to force stop
        await forceStopSimulation()
      }
    } else {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Detected simulation state is running, stopping...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
