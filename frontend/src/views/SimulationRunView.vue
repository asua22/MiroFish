...
// Get maxRounds from query params directly during Initializing to ensure child components get it immediately
...
const handleNextStep = () => {
  // Step3Simulation Component directly handles Report Generation and routing
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // Get config to obtain minutes_per_round
      try {
        const configRes = await getSimulationConfig(currentSimulationId.value)
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          minutesPerRound.value = configRes.data.time_config.minutes_per_round
          addLog(`Time config: Every round  ${minutesPerRound.value} minutes`)
        }
      } catch (configErr) {
        addLog(`Failed to get time config, using default: ${minutesPerRound.value}minutes/ rounds`)
      }
...
const loadGraph = async (graphId) => {
  // When simulating, auto-refresh hides full-screen loading to prevent flickering
...
onMounted(() => {
  ...
  // Record maxRounds config (value obtained from query params during Initializing)
  if (maxRounds.value) {
