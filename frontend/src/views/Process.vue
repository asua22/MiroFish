            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, the LLM analyzes the content and auto-generates an ontology (entity types + relation types) suitable for simulation
                </div>
              </div>
...
// Current project ID (may change from 'new' to actual)
...
const startGraphPolling = () => {
  // Always stop existing polling first to prevent leaks
  stopGraphPolling()
...
const fetchGraphData = async () => {
  ...
  try {
    // Get project info first to check status and obtain graph_id
    ...
    if (projectResponse.success) {
      ...
      // If project has failed, stop polling
      if (status === 'failed') {
        console.log('Project failed, stopping graph polling')
        stopGraphPolling()
        error.value = projectResponse.data.error || 'Project processing failed'
        return
      }
    ...
    if (statusCode === 403) {
      console.error('Zep Quota limit reached, stopping all polling')
      error.value = 'Zep API quota limit reached. Graph cannot be built or retrieved.'
      stopPolling()
      stopGraphPolling()
    } else if (statusCode === 429) {
      console.warn('Zep Rate limit reached, pausing graph polling for 60s')
      // Stop polling to respect the rate limit
      stopGraphPolling()
      error.value = 'Zep Rate limit reached. Updates paused for 60 seconds to allow API recovery...'
      
      // Clear any existing timeout to avoid multiple restarts
      if (graphRetryTimeout) clearTimeout(graphRetryTimeout)
      
      graphRetryTimeout = setTimeout(() => {
        // Only restart if the project hasn't failed in the meantime
        if (projectData.value?.status !== 'failed' && !error.value.includes('quota')) {
           error.value = ''
           startGraphPolling()
        }
      }, 60000) // 60s pause as suggested by Zep headers
    } else {
      console.log('Graph data fetch:', errorMsg || 'not ready')
    }
  }
}
...
const renderGraph = () => {
  ...
  // Draw edges (with clickable invisible wide lines)
  ...
  simulation.on('tick', () => {
    // Update all edge positions (including visible line and click area)
