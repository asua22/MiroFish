    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Starting...' : 'Start Result Report' }} 
...
// Format elapsed simulation time (calc by rounds and minutes/round)
...
const doStartSimulation = async () => {
  ...
  // Reset all states first to avoid previous simulation impact
  resetAllState()
...
// Track last platform rounds to detect changes and output logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect platform round changes independently and output logs
      ...
      // Detect if simulation Complete (via runner_status or platform status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: if backend runner_status lagging but platform reports complete
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one finished, check if all enabled platforms finished
  // Check if platform enabled by actions_count (count > 0 or running was true)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Do not auto-scroll, allow free viewing of timeline
