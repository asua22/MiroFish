            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but section may not be complete) -->
...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // Users icon - represents agent interview
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
  ...
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Database icon - represents entity query
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-queries - extract all
    ...
    // Extract relation chains - extract all
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract active facts - extract all
    ...
    // Extract historical/expired facts - extract all
    ...
    // Extract related entities - extract all
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Handle case where first question starts with "1."
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform markers (legacy format), use as whole answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - extract all
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift-query sub-question analysis'),
        ...
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No relation chains found')
      ])
    ])
  }
}
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Fallback to "number." format if no match
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no or single number found, return whole text
      if (matches.length <= 1) {
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch button (only show with real dual answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 titles since section title is displayed externally
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task complete, scroll to top; otherwise scroll to bottom to follow latest
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown, try to remove thinking process
