<template>
  ...
  >
    <!-- Background decoration: grid lines (visible when projects exist) -->
    ...
    <!-- Card container (visible when projects exist) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Start Simulation" and Step 5 "Deep Interaction" require running instances and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Calculate actual height: rows * cardHeight + (rows-1) * gap + bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count per row for centering
...
// Generate title from requirement (first 20 chars)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update target state regardless of animation to record latest
        ...
        // If animating, record state only, handle after animation
        if (isAnimating) return
        
        // If target state equals current, no need to handle
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounce to delay state switch, prevent flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs execution (might be overwritten)
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay before execution to prevent rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, viewport bottom shrinks up, needs more scroll to expand
...
// Listen to route change, reload data when returning to home page
...
onMounted(async () => {
  ...
  // Waiting for DOM render to initialize observer
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
/* Simplify display when no projects */
...
/* Create fixed-spacing square grid using CSS background */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position from top-left, expanding height only at bottom to preserve grid */
...
.cards-container {
  ...
  /* min-height calculated by JS, auto-adapts to card count */
}
...
/* Low saturation color scheme - Morandi */
