    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A few items are still processing, suggest to manually Refresh Graph later</span>
...
const showSimulationFinishedHint = ref(false) // Hints after simulation ends
const wasSimulating = ref(false) // Track if previously simulating

// Close end-of-simulation hint
...
// Toggle expand/collapse self-loop items
...
const renderGraph = () => {
  ...
  // Process edge data, calculate count and index between node pairs
  const edgePairCount = {}
  ...
  // Record the current edge index for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store all self-loop edges details
        }
      })
      return
    }
    ...
    // Calculate curvature: spread multi-edges, straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
      ...
      // If edge direction opposes standardized, flip curvature
      // Ensures all edges distribute in same reference frame, avoiding overlap from direction
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // Base distance 150, increase 40 per extra edge
    ...
    // Add gravity towards center to pull isolated node groups together
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Leave from right side, loop back around
    ...
    // Perpendicular offset, calculated by distance ratio to ensure visible curve
    // The more edges, the larger the offset ratio
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, +5% for each additional edge
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record pos, dont restart force layout (distinguishes click/drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect actual drag (movement > threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart layout only on actual drag detection
          d._isDragging = true
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (No rotation, better horizontal clarity)
...
// Watch edge labels visibility switch
