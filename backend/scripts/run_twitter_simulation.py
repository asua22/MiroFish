...
This script reads parameters from the configuration file to execute simulation, achieving full automation
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        print(
            f"LLM config: model={llm_model}, base_url={llm_base_url[:40] if llm_base_url else 'default'}..."
        )
    ...
    async def run(self, max_rounds: int = None):
        ...
        print(
            f"Wait-for-commands mode: {'Enabled' if self.wait_for_commands else 'Disabled'}"
        )
        print("=" * 60)
        ...
        if initial_posts:
            ...
            for post in initial_posts:
                ...
                try:
                    ...
                    print(
                        f"  Warning: Cannot create initial post for Agent {agent_id}: {e}"
                    )
...
def setup_signal_handlers():
    ...
    Give program chance to clean up resources (close database, environment, etc.)
    """
