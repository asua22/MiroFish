...
- After simulation completes, does not exit immediately, enters wait-for-commands mode
...
if sys.platform == "win32":
    ...
    # Reconfigure stdout to UTF-8 (fix console Chinese garbled text)
...
# Load project root .env file (containing LLM_API_KEY and other configs)
...
# Twitter available actions (not including INTERVIEW, INTERVIEW can only be triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
class ParallelIPCHandler:
    ...
    async def handle_interview(
        ...
        if not self.twitter_env and not self.reddit_env:
            self.send_response(
                command_id, "failed", error="No simulation environment available"
            )
            return False
    ...
    async def handle_batch_interview(
        ...
        Args:
            ...
            platform: Default platform (can be overridden by each interview item)
                - "twitter": Only interview Twitter platform
                - "reddit": Only interview Reddit platform
                - None/not specified: Each Agent interviewed on both platforms
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(
                command_id,
                "completed",
                result={"message": "Environment about to shutdown"},
            )
            return False
...
# Filtered non-core action types (these actions have low analysis value)
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl instead of "Agent_0" like codes
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last maximum rowid value (using rowid instead of created_at, because different platforms have different created_at formats)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keep only key fields (keep complete content, don't truncate)
            simplified_args = {}
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    - Boost config (optional): LLM_BOOST_API_KEY, LLLM_BOOST_BASE_URL, LLM_BOOST_MODEL_NAME
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError(
            "Missing API Key config, please set LLM_API_KEY in .env file at project root"
        )
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum simulation rounds (optional, used to truncate too long simulations)
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed rowid in database (using rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        # Get actual executed actions from database and log them
    ...
    log_info(
        f"Simulation loop completed! Time: {elapsed:.1f}s, Total actions: {total_actions}"
    )
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum simulation rounds (optional, used to truncate too long simulations)",
    )
    parser.add_argument(
        "--no-wait",
        action="store_true",
        default=False,
        help="Exit immediately after simulation completes, do not enter wait-for-commands mode",
    )
    ...
    log_manager.info(
        f"Wait-for-commands mode: {'Enabled' if wait_for_commands else 'Disabled'}"
    )
    log_manager.info("=" * 60)
    ...
    log_manager.info(f"Log structure:")
...
def setup_signal_handlers(loop=None):
    """
    Setup signal handlers, ensuring proper exit when receiving SIGTERM/SIGINT
    ...
    2. Give program chance to clean up resources (close database, environment, etc.)
    3. Then exit
    """
    ...
    def signal_handler(signum, frame):
        ...
        # If signal received again, force exit
        else:
            print("Force exit...")
            sys.exit(1)
