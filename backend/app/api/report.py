"""
Report API routes
Provides simulation report generation, retrieval, conversation and other interfaces
"""
...
def generate_report():
    ...
    try:
        ...
        if not state:
            return jsonify(
                {"success": False, "error": f"Simulation not found: {simulation_id}"}
            ), 404
        ...
        if not project:
            return jsonify(
                {"success": False, "error": f"Project not found: {state.project_id}"}
            ), 404
...
def get_generate_status():
    ...
    Request (JSON):
        {
            "task_id": "task_xxxx",         // optional,generateReturnsthetask_id
...
def list_reports():
    ...
    Query params:
        simulation_id: Filter by simulation_id (optional)
...
def resume_report(report_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify(
                {"success": False, "error": f"Report not found: {report_id}"}
            ), 404

        if report.status == ReportStatus.COMPLETED:
            return jsonify(
                {"success": False, "error": "Report is already completed"}
            ), 400
        ...
        simulation_requirement = (
            project.simulation_requirement if project else None
        ) or ""
        ...
        def run_resume():
            try:
                ...
                if result.status == ReportStatus.COMPLETED:
                    task_manager.complete_task(
                        task_id, result={"report_id": report_id, "status": "completed"}
                    )
                else:
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    if can_resume
                    else "Restarting from scratch (no outline found)",
                },
            }
        )
...
def chat_with_report_agent():
    ...
    try:
        ...
        graph_id = state.graph_id or project.graph_id
        if not graph_id:
            return jsonify({"success": False, "error": "Missing graphID"}), 400
...
def get_report_progress(report_id: str):
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "completed_sections": ["Executive Summary", "simulationbackground"],
    ...
    try:
        ...
        if not progress:
            return jsonify(
                {"success": False, "error": f"Report not foundor progress info unavailable: {report_id}"}
            ), 404
...
def get_report_sections(report_id: str):
    ...
    Frontend can poll this endpoint to get generated section content without waiting for entire report completion
...
def check_report_status(simulation_id: str):
    ...
    try:
        ...
        # Only unlocked after report completioninterview
...
def get_agent_log(report_id: str):
    ...
    Query params:
        from_line: Start from line (optional, default 0, for incremental fetch)
...
def get_console_log(report_id: str):
    ...
    This differs from the structured JSON log returned by agent-log endpoint,
