...
Transforms entities from the Zep knowledge graph into the Agent Profile format required by the OASIS simulation platform.

Optimizations:
1. Enriches node information by calling Zep retrieval functions.
2. Uses optimized prompts to generate highly detailed personas.
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "username": self.user_name,  # OASIS lib requires 'username' (no underscore)
            "name": self.name,
            "bio": self.bio,
    ...
    def to_twitter_format(self) -> Dict[str, Any]:
        ...
        profile = {
            "user_id": self.user_id,
            "username": self.user_name,  # OASIS lib requires 'username'
            "name": self.name,
            "bio": self.bio,
            "persona": self.persona,
...
class OasisProfileGenerator:
    ...
    Transforms entities from the Zep knowledge graph into Agent Profiles required by the OASIS simulation.

    Optimizations:
    1. Calls Zep graph retrieval to get rich context.
    2. Generates detailed personas (including basic info, professional background, personality, social media behavior, etc.).
    3. Distinguishes between individual entities and abstract group entities.
    """
    ...
    # Individual entity types (needs detailed persona generation)
    ...
    # Group/Institutional entity types (needs representative persona generation)
    GROUP_ENTITY_TYPES = [
        "university",
        "governmentagency",
        "organization",
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Uses Zep graph hybrid search to get rich information related to the entity.

        Zep does not have a built-in hybrid search interface; edges and nodes need
        to be searched separately and then results merged.
        Uses parallel requests to search simultaneously to improve efficiency.
        ...
        def search_edges():
            ...
            for attempt in range(max_retries):
                try:
                    if not self.zep_client or not hasattr(self.zep_client, "graph"):
                        return None
        ...
        try:
            # Parallel execution of edge and node search
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        """
        Builds the complete context information for the entity.
        ...
        if zep_results.get("facts"):
            # De-duplication: exclude existing facts
            new_facts = [f for f in zep_results["facts"] if f not in existing_facts]
            if new_facts:
                context_parts.append(
                    "### Facts Retrieved by Zep\n"
                    + "\n".join(f"- {f}" for f in new_facts[:15])
                )

        if zep_results.get("node_summaries"):
            context_parts.append(
                "### Related Nodes Retrieved by Zep\n"
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Determines if it is an individual type entity"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES

    def _is_group_entity(self, entity_type: str) -> bool:
        """Determines if it is a group/institutional type entity"""
    ...
    def _generate_profile_with_llm(
        ...
        """
        Uses LLM to generate a very detailed persona.

        Differentiated by entity type:
        - Individual entities: Generates specific character settings.
        - Group/Institutional entities: Generates representative account settings.
        """
        ...
        # Try multiple generations until success or maximum retries reached
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # No max_tokens set, let the LLM be free
                )
                ...
                if finish_reason == "length" and content:
                    logger.warning(
                        f"LLM output truncated (attempt {attempt + 1}), trying to fix..."
                    )
        ...
        logger.warning(
            f"Failed to generate persona with LLM ({max_attempts} attempts): {last_error}, using rule-based generation"
        )
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Fixes truncated JSON (output truncated by max_tokens limit)"""
        import re
        ...
        # Simple check: if the last quote is not followed by comma or closing bracket, it might be truncated
    ...
    def _try_fix_json(
        ...
        if json_match:
            ...
            # Find all string values and replace newlines with spaces
            def fix_string_newlines(match):
                s = match.group(0)
                # Replace actual newlines within string to space
                s = s.replace("\n", " ").replace("\r", " ")
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Gets system prompt"""
        # Removed Chinese restriction, requiring matching input language
        base_prompt = "You are an expert in social media user persona generation. Generate detailed, realistic personas for public opinion simulation, maximizing adherence to existing real-world conditions. You must return valid JSON format, and all string values must not contain unescaped newlines. Detect the primary language of the input data and use this exact language for all your output."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        """Builds the detailed persona prompt for an individual entity"""
        ...
        return f"""Generate a detailed social media user persona for the entity, maximizing adherence to existing real-world conditions.
...
Please generate valid JSON, including the following fields:

1. bio: Social media bio (approx 200 characters).
2. persona: Detailed persona description (approx 2000 characters of pure text), including:
   - Basic Info (age, occupation, educational background, location)
   - Background (key experiences, association with the event, social connections)
   ...
   - Stances/Views (attitude towards topics, content that might trigger/move them)
   - Unique Features (catchphrases, special experiences, personal hobbies)
   - Personal Memories (an important part of the persona, introducing the individual's association with the event and their existing actions/reactions in the event)
3. age: Age (integer).
4. gender: Gender, must be one of: "male", "female", "other".
5. mbti: MBTI type (e.g., INTJ, ENFP).
6. country: Country (use the same language as input).
...
- Use the same language as the input for all descriptions (except gender, which must be "male", "female", or "other").
...
- age must be a valid integer."""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the institutional/group entity, maximizing adherence to existing real-world conditions.
...
1. bio: Official account bio, approx 200 characters, professional.
2. persona: Detailed account settings description (approx 2000 characters of pure text), including:
   - Institutional Information (official name, nature, background, main functions)
   - Account Positioning (type, target audience, core function)
   - Speaking Style (language characteristics, common expressions, forbidden topics)
   - Posting Content Characteristics (content types, frequency, active hours)
   - Stance/Attitudes (official position on core topics, handling of controversies)
   - Special Instructions (group portrait represented, operational habits)
   - Institutional Memory (an important part of the persona, introducing the association with the event and the entity's existing actions/reactions in the event)
3. age: Set to 30 (virtual age for institutional accounts).
4. gender: Set to "other" (for institutions).
...
- Persona must be a coherent text description, no newlines.
- Use the same language as the input for all descriptions.
- age must be integer 30, gender must be "other".
- The account's speech must fit its institutional identity."""
    ...
    def _generate_profile_rule_based(
        ...
        """Uses rules to generate a basic persona"""
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "mbti": "ISTJ",  # Style: rigorous/conservative
                "country": "US",
                "profession": "Media",
                "interested_topics": [
                    "General News",
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: Real-time writing file path (if provided, write once per generation)
        ...
        profiles: List[Optional[OasisAgentProfile]] = [
            None
        ] * total  # Pre-allocate list to maintain order
        completed_count = [0]  # Use list to modify in closure
        lock = Lock()

        # Auxiliary function for real-time file writing
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                logger.error(
                    f"Failed to generate persona for entity {entity.name}: {str(e)}"
                )
        ...
        logger.info(
            f"Starting parallel generation of {total} Agent personas (parallel count: {parallel_count})..."
        )
        print(f"\n{'=' * 60}")
        print(
            f"Starting Agent persona generation - Total {total} entities, parallel count: {parallel_count}"
        )
        print(f"{'=' * 60}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    logger.error(
                        f"Exception occurred while processing entity {entity.name}: {str(e)}"
                    )
                    with lock:
                        completed_count[0] += 1
                    ...
                    # Real-time file write (even for fallback)
                    save_profiles_realtime()

        print(f"\n{'=' * 60}")
        print(
            f"Persona generation complete! Total generated {len([p for p in profiles if p])} Agents"
        )
        print(f"{'=' * 60}\n")

        profiles_data: List[OasisAgentProfile] = [p for p in profiles if p is not None]
    ...
    def _print_generated_profile(
        ...
        # Construct full output content (no truncation)
        topics_str = (
    ...
    def save_profiles(
        ...
        """
        Save Profile to file (selects correct format based on platform)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile as CSV format (compliant with official OASIS requirements)
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Keep English mapping, maintain neutral
        gender_map = {
            "male": "male",
            "female": "female",
            "other": "other",
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Uses the same format as to_reddit_format(), ensuring OASIS can correctly read it.
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                else idx,  # Critical: must contain user_id
                ...
                # OASIS required fields - ensure they have default values
    ...
    # Alias for old method name, for backward compatibility
    def save_profiles_to_json(
        ...
        logger.warning(
            "save_profiles_to_json is deprecated, please use save_profiles method"
        )
