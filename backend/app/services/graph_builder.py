...
class GraphBuilderService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        # Use dynamic key loading to ensure it picks up .env changes
        self.api_key = api_key or Config.get_zep_api_key()
        if not self.api_key:
            raise ValueError("ZEP_API_KEY not configured")

        # Masked key for logging (e.g. z_...1234)
        masked_key = f"{self.api_key[:3]}...{self.api_key[-4:]}"
        logger.info(f"Initialized GraphBuilderService with API Key: {masked_key}")
    ...
    def _build_graph_worker(
        ...
        try:
            ...
            self.task_manager.update_task(
                task_id, progress=60, message="Waiting for Zep data processing..."
            )
            ...
            # 6. Get graph info
            self.task_manager.update_task(
                task_id, progress=90, message="Getting graph info..."
            )
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # Required by Zep SDK, warning from dynamic class creation, safe to ignore
        warnings.filterwarnings("ignore", category=UserWarning, module="pydantic")

        # Zep reserved name, cannot be attribute name
        RESERVED_NAMES = {
            "uuid",
            "name",
            "group_id",
        ...
        # Dynamically create entity type
        entity_types = {}
        for entity_def in ontology.get("entity_types", []):
            name = entity_def["name"]
            description = entity_def.get("description", f"A {name} entity.")

            # Create attribute dict and type hints (Pydantic v2 req)
        ...
        # Dynamically create edge type
        edge_definitions = {}
        for edge_def in ontology.get("edge_types", []):
            name = edge_def["name"]
            description = edge_def.get("description", f"A {name} relationship.")

            # Create attribute dict and type hints
            attrs = {"__doc__": description}
            annotations = {}
            ...
            for st in edge_def.get("source_targets", []):
                sources = st.get("source", "Entity")
                targets = st.get("target", "Entity")
                    ...
                    for t in targets:
                        source_targets.append(
                            EntityEdgeSourceTarget(
                                source=s if isinstance(s, str) else "Entity",
                                target=t if isinstance(t, str) else "Entity",
                            )
                        )
    ...
    def add_text_batches(
        ...
        """Batch add text to graph, returns all episode uuid list"""
        episode_uuids = []
        total_chunks = len(chunks)
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            try:
                ...
                error_msg = str(e)
                if status_code == 403:
                    error_msg = f"Zep API quota limit reached (403 Forbidden): {str(e)}"
                elif status_code == 429:
                    error_msg = f"Zep API rate limit reached (429 Too Many Requests): {str(e)}. Please wait before retrying."
                
                if progress_callback:
                    progress_callback(f"Batch {batch_num} failed: {error_msg}", 0)
    ...
    def _wait_for_episodes(
        ...
        """Wait all episodes processing (query via processed status)"""
        ...
        if progress_callback:
            progress_callback(
                f"Starting wait for {total_episodes} text chunks processing...", 0
            )
        ...
        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"Some chunks timeout, completed {completed_count}/{total_episodes}",
                        completed_count / total_episodes,
                    )
                break

            # Check each episode process status
        ...
        if progress_callback:
            progress_callback(
                f"Processing completed: {completed_count}/{total_episodes}", 1.0
            )
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Get full graph data (with details)

        Args:
            graph_id: Graph ID

        Returns:
            Dict with nodes/edges, including time info & properties
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # Create node mapping to get names
        node_map = {}
        for node in nodes:
