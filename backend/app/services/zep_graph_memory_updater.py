...
Dynamically updates agent activities during simulation into the Zep graph.
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Converts activity to a text description for Zep.
        ...
        """
        # Generate descriptions based on different action types
        action_descriptions = {
    ...
    def _describe_create_comment(self) -> str:
        """Create comment - includes content and post context"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_generic(self) -> str:
        # Generates a generic description for unknown action types
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation actions log files and updates new agent activities to the Zep graph in real-time.
    Groups by platform and batches activities to Zep after accumulating BATCH_SIZE entries per platform.

    All meaningful actions are updated to Zep, with action_args containing the full context:
    - Original post content for likes/dislikes
    - Original post content for reposts/quotes
    - Usernames for follows/mutes
    - Original comment content for like/dislike comments
    """
    ...
    # Send interval (seconds) to prevent too fast requests
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Platform-specific activity buffer
    ...
    def add_activity(self, activity: AgentActivity):
        """
        Adds an agent activity to the queue
        ...
        action_args contain full context information.
    ...
    def add_activity_from_dict(self, data: Dict[str, Any], platform: str):
        """
        Adds activity from dictionary data
    ...
    def _worker_loop(self):
        """Background worker loop - batches activities to Zep per platform"""
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            ...
                            # Sleep interval to prevent fast request hammering
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Batches activities and sends them to Zep graph (merged as a single text)
        ...
        # Merge activities into single text separated by newline
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logger.info(
                    f"Successfully sent {len(activities)} {display_name} activities to graph {self.graph_id}"
                )
                ...
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(
                        f"Failed to send batch to Zep (Attempt {attempt + 1}/{self.MAX_RETRIES}): {e}"
                    )
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(
                        f"Failed to send batch to Zep after {self.MAX_RETRIES} attempts: {e}"
                    )
                    self._failed_count += 1

    def _flush_remaining(self):
        """Sends remaining activities in queue and buffers"""
        # Process remaining activities in queue and add to buffer
        ...
        # Send remaining activities in platform buffers (even if less than BATCH_SIZE)
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(
                        f"Sending remaining {len(buffer)} activities for platform {display_name}"
                    )
...
class ZepGraphMemoryManager:
    """
    Manages Zep graph memory updater instances for multiple simulations.
    ...
    def get_updater(cls, simulation_id: str) -> Optional[ZepGraphMemoryUpdater]:
        """Gets updater for a simulation"""
        return cls._updaters.get(simulation_id)

    @classmethod
    def stop_updater(cls, simulation_id: str):
        """Stops and removes updater for a simulation"""
        with cls._lock:
            if simulation_id in cls._updaters:
                cls._updaters[simulation_id].stop()
    ...
    def get_all_stats(cls) -> Dict[str, Dict[str, Any]]:
        """Gets statistics for all updaters"""
        return {
