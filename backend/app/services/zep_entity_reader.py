...
Read nodes from Zep graph, filter pre-defined entity types
"""
...
class EntityNode:
    ...
    def get_entity_type(self) -> Optional[str]:
        """Get entity types (exclude default Entity label)"""
...
class ZepEntityReader:
    ...
    def _call_with_retry(
        ...
        Args:
            func: Function to execute (lambda or callable, no parameters)
            operation_name: Operation name for logging
            max_retries: max retries (default 3 times)
            ...
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        ...
                        f"{delay:.1f}seconds to retry..."
                    )
                    time.sleep(delay)
                    delay *= 2  # Exponential backoff
                else:
                    logger.error(
                        f"Zep {operation_name} at {max_retries} attempts still failed: {str(e)}"
                    )
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all edges of graph (pagination)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all related edges for specific node (with retry)
    ...
    def filter_defined_entities(
        ...
        Filter logic:
        - If node labels only has "Entity", it does not match predefined type, skip
        - If node labels has other than "Entity" and "Node", matches predefined type, keep

        Args:
            graph_id: Graph ID
            defined_entity_types: Predefined entity type list (optional, filters to these types)
            enrich_with_edges: Whether to get related edge info for each entity

        Returns:
            FilteredEntities: Filtered entities set
        """
        ...
        # Get all edges (for relation lookups)
        all_edges = self.get_all_edges(graph_id) if enrich_with_edges else []

        # Build node UUID to data mapping
        ...
        for node in all_nodes:
            ...
            # If predefined type specified, check match
        ...
        logger.info(
            f"Filter complete: Total nodes {total_count}, matching {len(filtered_entities)}, "
    ...
    def get_entity_with_context(
        ...
        """
        Get single entity and full context (edges and nodes, with retry)
        ...
        try:
            ...
            # Get all nodes for associated search
            all_nodes = self.get_all_nodes(graph_id)
            node_map = {n["uuid"]: n for n in all_nodes}
