...
Used for IPC between Flask backend and simulation scripts

Simple command/response via file system:
1. Flask writes commands to commands/ dir
2. Script polls commands, executes, and writes response to responses/ dir
3. Flask polls responses dir for results
"""
...
class SimulationIPCClient:
    ...
    Used to send command to simulation process and await response
    """
    ...
    def send_command(
        ...
        # Timeout
        logger.error(f"Wait for IPC response timed out: command_id={command_id}")
    ...
    def send_interview(
        ...
        Args:
            ...
            platform: Specific platform (opt)
                - "twitter": Twitter only
                - "reddit": Reddit only
                - None: Both platforms in dual mode, active platform in single mode
            timeout: 超时时间
    ...
    def send_batch_interview(
        ...
        Args:
            interviews: list, elems={"agent_id": int, "prompt": str, "platform": str(opt)}
            platform: Default platform (opt, overridden by interview items)
                - "twitter": Default to interview Twitter platform only
                - "reddit": Default to interview Reddit platform only
                - None: Both platforms per Agent in dual mode
    ...
    def check_env_alive(self) -> bool:
        ...
        Determined via env_status.json file checking
        """
...
class SimulationIPCServer:
    """
    Simulation IPC Server (for script side)

    Poll commands dir, execute, return response
    """
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        """
        Poll commands dir, return first pending command
